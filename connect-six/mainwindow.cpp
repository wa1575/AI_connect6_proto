...
void MainWindow::startNetworkGame(){
    ...
    connect (this->array->server, SIGNAL (connectionStatus (int)), this, SLOT (setStatusBar (int)));//누군가가 연결 / 연결 해제 된 상태 표시 줄을 가리켜 야합니다.
...
void MainWindow::ConnectToGame(){
	...
	if (ok1 && ok2 && !address.isEmpty() && port!=0){
        ...
        connect (this->array->client, SIGNAL (connectionStatus (int)), this, SLOT (setStatusBar (int)));// 누군가가 연결/연결 해제 된 상태 표시 줄을 가리켜 야합니다.
